...
1. **Replay of captured genuine strokes passes verification.** The scoring path has no anti-replay defense. Exact-copy stroke data yields a DTW distance near zero and identical features, fusing to a score around 100 ([signature-fusion.ts](../packages/backend/src/features/comparison/signature-fusion.ts)). The mitigations the doc previously credited are weak: `capturedAt` is client-supplied so the freshness check is trivially satisfied; challenges are not bound to the verifying username (`verifyFull` never cross-checks the challenge's session owner); and because `shapes` is a client-assembled array, one capture per shape type can be reordered to match any challenge order. `POST /api/verify/full` and `POST /api/challenge` are both unauthenticated. An attacker who observes a user's genuine signature + shape strokes once can replay them indefinitely.
   - **No robust server-only fix exists.** Replay shares its root cause with the "synthetic strokes not rejected" finding below: the server receives JSON it cannot distinguish from a live human capture. Approaches that look tempting but do **not** close it:
     - *Challenge-to-user binding* — the attacker replays the victim's strokes against the victim's account and can mint a challenge for any username (`/api/challenge` is unauthenticated). Worth doing for hygiene, but irrelevant to replay.
     - *Per-capture nonce* — the nonce is just another field in attacker-controlled JSON; a replay attacker reads the fresh nonce and staples it onto the replayed strokes. A nonce only helps if the capture path is attested and signs the payload.
     - *Exact-match rejection* — catches literal byte-for-byte resubmission only. Scoring is deliberately tolerant of natural variation, so sub-pixel / sub-millisecond jitter produces a non-identical payload that still scores ~99.
   - **Real fix:** a trusted/attested capture path — the payload signed by a device or app-attestation key (Play Integrity / App Attest) so the server can reject anything not produced by a genuine in-app capture. This is a project, not a patch, and is the same dependency as hardening synthetic-stroke rejection.
   - **Best server-only mitigation (partial):** a *novelty check* — reject an attempt that is *too* similar to a stored enrollment sample (an upper similarity bound alongside the existing lower bound). Humans never reproduce themselves to ~100, so this also catches perturbed replay. It is a genuine calibration problem — it trades replay-resistance against rejecting consistent genuine users — and needs a real genuine-vs-replay distribution before the bound can be set. Prototype behind a flag; do not ship as "solved."
